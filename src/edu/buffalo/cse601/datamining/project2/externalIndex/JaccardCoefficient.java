...
public class JaccardCoefficient {
	...
	public JaccardCoefficient(HashMap<String, Integer> groundTruth, HashMap<String, Integer> clusteringSolution) {
		...
		int i = 0;
		for (Entry<String, Integer> entry1 : groundTruth.entrySet()) {
			int clusterId1 = entry1.getValue();
			int j = 0;
			for (Entry<String, Integer> entry2 : groundTruth.entrySet()) {
		...
		i = 0;
		for (Entry<String, Integer> entry1 : clusteringSolution.entrySet()) {
			int clusterId1 = entry1.getValue();
			int j = 0;
			for (Entry<String, Integer> entry2 : clusteringSolution.entrySet()) {
				int clusterId2 = entry2.getValue();
				if ((clusterId1 != -1 && clusterId2 != -1) && (clusterId1 == clusterId2)) {
					clusterMatrix[i][j] = 1;
					clusterMatrix[j][i] = 1;
	...
	public JaccardCoefficient(TreeMap<Integer, Integer> groundTruth, Map<Integer, Integer> clusteringSolution) {
		groundTruthMatrix = new int[groundTruth.size()][groundTruth.size()];
		clusterMatrix = new int[clusteringSolution.size()][clusteringSolution.size()];
...
//					System.out.println(i + "\t" + j);
		...
		return (double) M11 / (double) (M11 + M10);
	}
